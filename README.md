...
* **Unified LiveView Component:** Uses `lib/social_scribe_web/live/meeting_live/crm_modal_component.ex`
  with provider module `SocialScribe.CRM.Providers.Hubspot.Provider`
* **Contact Search:** List-on-open + debounced search via `SocialScribe.CRM.Providers.Hubspot.Provider.list_contacts/2`
...
* **Form Submission:** Batch-updates selected contact properties via `SocialScribe.CRM.Providers.Hubspot.Api.update_contact/3`
...
* **Unified LiveView Component:** Uses `lib/social_scribe_web/live/meeting_live/crm_modal_component.ex`
  with provider module `SocialScribe.CRM.Providers.Salesforce.Provider`
...
* **AI Suggestions:** `SocialScribe.CRM.Providers.Salesforce.Suggestions.generate_suggestions/3` combines transcript context and Salesforce Contact field metadata to generate update candidates
* **Field Mapping + Validation:** Users can remap suggestion fields, edit values, and submit only changed fields
* **Form Submission:** Selected updates are validated/coerced by field type and patched via `SocialScribe.CRM.Providers.Salesforce.Api.update_contact/3`

---

## 🧩 Add Another CRM Integration

This project now uses a provider-centric CRM architecture under `lib/social_scribe/crm/providers/`.

### 1) Implement the CRM contracts

Create provider-specific modules:

* `lib/social_scribe/crm/providers/<provider>/api.ex`
  * Implement `SocialScribe.CRM.ApiBehaviour`
  * Add `SocialScribe.CRM.DescribeFieldsBehaviour` only if the CRM supports field discovery
* `lib/social_scribe/crm/providers/<provider>/suggestions.ex`
  * Add transcript -> CRM suggestion generation/normalization
* `lib/social_scribe/crm/providers/<provider>/provider.ex`
  * Implement `SocialScribe.CRM.Provider` for UI-facing behavior:
    * credentials selection
    * contact list/search
    * mapping defaults
    * form state/update validation
    * provider-specific error formatting/capabilities

### 2) Register runtime API implementation

Add app config for API injection (keeps tests/mocks easy):

```elixir
config :social_scribe, :<provider>_api, SocialScribe.CRM.Providers.<Provider>.Api
```

### 3) Wire the provider into Meeting LiveView

In `SocialScribeWeb.MeetingLive.Show`:

* load credentials through `<Provider>.Provider.list_credentials/1`
* pass `provider={SocialScribe.CRM.Providers.<Provider>.Provider}` into `CrmModalComponent`
* keep using the generic CRM `handle_info` message flow (`:crm_search`, `:crm_generate_suggestions`, `:crm_apply_updates`)

### 4) Add route + entry button

* Add a route live action in `router.ex` (for example `/meetings/:id/<provider>`)
* Add integration card/button in `show.html.heex` that patches to that action

### 5) Add tests

* API tests for `<provider>/api.ex`
* Suggestions tests for `<provider>/suggestions.ex`
* Mox-backed modal integration tests through unified `CrmModalComponent`

This keeps the UI shared and pushes CRM-specific differences into provider modules only.
